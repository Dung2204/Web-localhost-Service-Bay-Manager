...
def update_task(task_id):
    ...
    if current_task_dict.get('paymentStatus') == 'paid' and not is_admin:
         # [ĐẶC BIỆT]: Cho phép yêu cầu thanh toán nếu đang là 'pending' (vì admin duyệt)
         if not (is_payment_request and current_task_dict.get('paymentStatus') == 'pending'):
    ...
    # --- [UPDATE] LOGIC QUYỀN: BỔ SUNG QUYỀN THANH TOÁN VÀ YÊU CẦU XÓA (CẬP NHẬT THEO YÊU CẦU MỚI) ---
    ...
    if not (is_admin or (is_owner and is_task_today) or can_request_payment or can_request_delete):
...
if __name__ == '__main__':
    ...
    print(">>>")
    print(">>> Truy cập http://127.0.0.1:5000 trong trình duyệt.")
    print(">>> Nhấn CTRL+C để dừng máy chủ.")
    print("-----------------------------------------------------")
    # host='0.0.0.0' cho phép các máy khác trong mạng LAN truy cập
    # debug=True tự động khởi động lại khi sửa code, nhưng không dùng cho production
